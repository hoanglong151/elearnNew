...
const BookingLessonModal = (
	...
	const [APIBookState, setAPIBookState] = useState(true);
	...
	const getLessonToBookingAPI = async () => {
		...
		console.log("=============BookingLessonModal RES");
		console.log(res);
		
		if (res.Code === 1) {
			setAPIBookState(true);
			setBookState({
				...res.Data,
				Code: 1,
			});
		} else if (res.Code === 0) {
			setAPIBookState(false);
		}		
	};
	...
	return APIBookState === false ? (
		// bookState && bookState.Code === 0 ? (
			<div
											...
											<div className="required-list mg-t-15 bd-t pd-t-15">
												<div className="required-text-box metronic-form">
													<label className="tx-medium">
														Ghi chú cho giáo viên/Note for teacher:
													</label>
													<label className="tx-danger d-block">
														Vui lòng viết bằng Tiếng Anh (tối đa 200 ký tự)/Please take note in English (maximum 200 characters) 
													</label>
